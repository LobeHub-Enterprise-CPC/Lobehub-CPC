...
it('scans past a full page of offline jobs, preserves FIFO and revisits them after recovery', async () => {
  vi.clearAllMocks();
  ...
  try {
    ...
    probe.mockImplementation(async (cwd: string) => {
      if (!recovered && cwd === '/blocked') throw new Error('DEVICE_OFFLINE');
      return cwd;
    });
