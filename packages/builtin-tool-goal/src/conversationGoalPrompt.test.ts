...
vi.mock('@lobechat/business-const', () => ({ BRANDING_NAME: 'Example Work' }));
...
describe('withConversationGoalPrompt', () => {
  it('uses the deployment brand in agent-facing instructions', () => {
    expect(conversationGoalPrompt).toContain("Example Work's /goal command");
    expect(conversationGoalPrompt).toContain('Example Work Goal');
    expect(conversationGoalPrompt).not.toContain('LobeHub');
  });
