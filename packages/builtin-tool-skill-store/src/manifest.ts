...
export const SkillStoreManifest: BuiltinToolManifest = {
  ...
  meta: {
    ...
    description: `Browse and install agent skills from the ${BRANDING_NAME} marketplace. MUST USE this tool when users mention: "SKILL.md", "${BRANDING_NAME} Skills", "skill store", "install skill", "search skill", or need extended capabilities. A \`lobehub.com/skills/{identifier}\` URL already contains the identifier — activate this tool and call importFromMarket with it; use importSkill for any other skill URL. Do not read such a page to look up install steps. The marketplace CLI (\`npx @lobehub/market-cli\`) is a last resort for agents without this tool, not the first step.`,
    title: 'Skill Store',
  },
